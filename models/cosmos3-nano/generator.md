...
Temporal chunking and rectified-flow sampling are compatible: a flow sampler can generate each conditional block of an autoregressive video. The pinned Framework exposes video_temporal_causal, per-latent-frame noise under causal_training_strategy="diffusion_forcing", and action/vision supertoken packing. The separate Transfer path also implements overlapping continuation through generate_transfer_sample. These are configuration- and task-specific code surfaces, not a claim of real-time streaming for the base Nano checkpoint. [C3-FW, cosmos_framework/model/generator/omni_mot_model.py; cosmos_framework/data/generator/sequence_packing/temporal_causal.py; cosmos_framework/inference/transfer.py]
