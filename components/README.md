...
The four entries are non-exclusive. For example, action-conditioned video planning may require representation, dynamics, reasoning, and generative-modeling syntheses, together with canonical action, planning, and evaluation pages from Foundations. Action representation and world-model–policy interfaces are not yet independent Component entries.
