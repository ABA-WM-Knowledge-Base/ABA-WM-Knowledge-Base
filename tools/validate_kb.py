...
SOURCES_PATH = MODEL_ROOT / "sources.yaml"

FRONTMATTER = re.compile(r"\A---\s*\n(.*?)\n---\s*\n", re.DOTALL)
MARKDOWN_LINK = re.compile(r"(?<!!)\[[^\]]+\]\(([^)]+)\)")
SOURCE_REFERENCE = re.compile(
    r"(?<!RQ-)\b((?:C3|C1|R1|T1|P25|LOCAL|NVIDIA)-[A-Z0-9-]+)\b"
)
...
def validate_sources(errors: list[str]) -> set[str]:
    """Validate source identities, local locators, and file integrity."""
    ...
    if data.get("schema_version") != schema.get("schema_version"):
        errors.append(
            "sources.yaml: schema_version does not match _schema/sources.schema.yaml"
        )

    collection_missing = set(collection_contract["required"]) - data.keys()
    if collection_missing:
        errors.append(
            f"sources.yaml: missing collection fields {sorted(collection_missing)}"
        )
    ...
    if collection_unknown:
        errors.append(
            f"sources.yaml: undeclared collection fields {sorted(collection_unknown)}"
        )
    ...
    if "local_path_policy" in data:
        ...
        if not isinstance(policy, dict):
            errors.append("sources.yaml: local_path_policy must be a mapping")
        else:
            policy_missing = policy_required - policy.keys()
            if policy_missing:
                errors.append(
                    "sources.yaml: local_path_policy missing fields "
                    f"{sorted(policy_missing)}"
                )
            policy_unknown = set(policy) - policy_required
            if policy_unknown:
                errors.append(
                    "sources.yaml: local_path_policy has undeclared fields "
    ...
    for index, source in enumerate(sources):
        if not isinstance(source, dict):
            errors.append(f"sources.yaml: source #{index} is not a mapping")
            continue
        ...
        if unknown_fields:
            errors.append(
                f"source #{index}: undeclared fields {sorted(unknown_fields)}"
            )

        source_id = source.get("id")
        if not isinstance(source_id, str) or not source_id:
            errors.append(f"source #{index}: id must be a non-empty string")
        elif source_id in source_ids:
            errors.append(f"sources.yaml: duplicate source id {source_id}")
        else:
        ...
        if source_type == "local_artifact":
            ...
            if local_missing:
                errors.append(
                    f"source {source_id}: local artifact missing {sorted(local_missing)}"
                )
        if source_type == "paper" and source.get("local_path"):
            paper_missing = cached_paper_required - source.keys()
            if paper_missing:
                errors.append(
                    f"source {source_id}: cached paper missing {sorted(paper_missing)}"
                )
        ...
        if related_path:
            related_file = (MODEL_ROOT / str(related_path)).resolve()
            if not related_file.is_file():
                errors.append(
                    f"source {source_id}: related_path does not exist: {related_path}"
                )
...
def validate_pages(
    ...
    retrieval_labels = tuple(schema["model_topic_page"]["retrieval_fields"])
    ...
    for path in sorted(ROOT.rglob("*.md")):
        ...
        if path.parent == MODEL_ROOT:
            ...
            if path.name != "README.md" and len(text) < 1200:
                errors.append(
                    f"{relative(path)}: model page is too thin ({len(text)} chars)"
                )
...
def validate_structure(schema: dict, errors: list[str]) -> None:
    ...
    required_model_files = set(schema["model_entry_required_files"])
    actual_model_files = {
    ...
    for reserved_part in (ROOT / "foundations", ROOT / "papers"):
        files = [path for path in reserved_part.rglob("*") if path.is_file()]
        if {path.name for path in files} != {"README.md"}:
            errors.append(
                f"{reserved_part.name}/ must remain content-empty in this version; "
                f"found {[relative(path) for path in files]}"
            )
...
def validate_agent_index(schema: dict, errors: list[str]) -> None:
    ...
    def validate_model_path(owner: str, target: object) -> None:
        if not isinstance(target, str):
            ...
            return
        path = Path(target)
        if path.is_absolute() or not (MODEL_ROOT / path).resolve().is_file():
            errors.append(
...
def main() -> int:
    ...
    try:
        ...
        if orphan_sources:
            errors.append(
                f"sources.yaml: unreferenced source IDs {sorted(orphan_sources)}"
            )
    ...
    source_count = len(load_yaml(SOURCES_PATH).get("sources", []))
    profile_count = len(load_yaml(AGENT_INDEX_PATH).get("retrieval_profiles", []))
    print(
        "KB validation passed: "
        f"3 content parts, {page_count} pages, {model_pages} Cosmos3-Nano pages, "
        f"{source_count} sources, {profile_count} knowledge-guidance retrieval profiles."
    )
    return 0
